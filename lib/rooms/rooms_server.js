...
/**
 * Wether or not the room join is denied
 * @param {Object} data Data of the message
 * @param {Socket} from From socket
 */
Streamy.Rooms.denyJoin = function(data, from) {
  return false;
};
...
/**
 * Wether or not the room leave is denied
 * @param {Object} data Data of the message
 * @param {Socket} from From socket
 */
Streamy.Rooms.denyLeave = function(data, from) {
  return false;
};
...
/**
 * Wether or not the room message is denied
 * @param {Object} data Data of the message
 * @param {Socket} from From socket
 */
Streamy.Rooms.denyMessage = function(data, from) {
  return false;
};
...
Streamy.on('__join__', function(data, from) {
  ...
  if(!Streamy.Rooms.allowJoin(data, from) || Streamy.Rooms.denyJoin(data, from))
      return;
...
Streamy.on('__leave__', function(data, from) {
  ...
  // Check if the server allows joining
  if(!Streamy.Rooms.allowLeave(data, from) || Streamy.Rooms.denyLeave(data, from))
      return;
...
Streamy.on('__room__', function(data, from) {
  ...
  if(!Streamy.Rooms.allowMessage(data, from) || Streamy.Rooms.denyMessage(data, from))
      return;
