...
Meteor.default_server.stream_server._streamyConnectionId = 'default';
  ...
  Meteor.default_server.stream_server.register(function onNewConnected(socket) {
    ...
    socket.on('close', function onSocketClosed() {
      if(handlers_registered) {
        ...
        _.each(self.disconnectHandlers(Meteor.default_server.stream_server), function forEachDisconnectHandler(cb) {
          cb.call(self, socket);
        });
      }
    });
    ...
    socket.on('data', function onSocketData(raw_data) {
      ...
      if(!handlers_registered && socket._meteorSession) {
        ...
        _.each(self.connectHandlers(Meteor.default_server.stream_server), function forEachConnectHandler(cb) {
          cb.call(self, socket);
        });

        // Add each handler to the list of protocol handlers
        _.each(self.handlers().default, function forEachHandler(cb, name) {
